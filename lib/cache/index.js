...
function connectToRedis( timeout, name ) {
  ...
  name = name || 'Unnamed';
  var to;
  var redisStr = process.env.REDIS_TLS_URL || process.env.REDIS_URL || 'redis://127.0.0.1:6379';

  console.log('Connecting to', redisStr);

  const isTLS = redisStr.indexOf('rediss:') >= 0;
  let connection

  if (isTLS) {
    connection = redisLib.createClient({
      url: redisStr,
      tls: {
    ...
    connection = redisLib.createClient( redisURL.port, redisURL.hostname, {no_ready_check: true} );
    if ( redisURL.auth ) {
