...
exports.create = function create( options, callback ) {
	...
	const unknownPathRateLimiter = rateLimit({
		...defaultRateLimitOptions,
		ruleName: 'root',
		windowMs: 1000 * 60 * 5,
		limit: 100,
	...
	app.get('/', unknownPathRateLimiter, rootHandler)
	// This catch app handler must be the last route
	app.get('/*', unknownPathRateLimiter, catchallHandler)
...
function catchallHandler (req, res) {
	res.set('Cache-Control', 'max-age=86400, stale-while-revalidate=259200, stale-if-error=604800');
	return notFound(res)
}

function badRequest (res, message) {
	return res.set('Cache-Control', 'max-age=86400, stale-while-revalidate=259200, stale-if-error=604800, immutable')
	.status(400)
